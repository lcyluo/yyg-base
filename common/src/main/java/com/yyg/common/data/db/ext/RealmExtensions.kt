...
inline fun <reified T : RealmModel> querySorted(
    fieldName: String,
    order: Sort,
    query: Query<T>
): List<T> {
 ...
 */
fun <T : RealmModel> T.querySorted(
    ...
    getRealmInstance().use { realm ->
        val result = realm.where(this.javaClass).withQuery(query).findAll()
            .sort(fieldName.toTypedArray(), order.toTypedArray())
        return realm.copyFromRealm(result)
    }
}
 ...
 */
inline fun <reified T : RealmModel> T.queryAndUpdate(
    noinline query: Query<T>,
    noinline modify: (T) -> Unit
) {
...
fun <T : RealmModel> T.hasPrimaryKey(realm: Realm): Boolean {
    requireNotNull(realm.schema.get(this.javaClass.simpleName)) { this.javaClass.simpleName + " is not part of the schema for this Realm. Did you added realm-android plugin in your build.gradle file?" }
...
inline fun <reified T : RealmModel> hasPrimaryKey(realm: Realm): Boolean {
    ...
    requireNotNull(realm.schema[simpleName]) { "$simpleName is not part of the schema for this Realm. Did you add realm-android plugin in your build.gradle file?" }
