...
inline fun <reified T : RealmModel> queryAsSingle(noinline query: Query<T>) =
    singleQuery(query = query)

/**
 * Query for sorted entities and observe changes returning a Single.
 */
fun <T : RealmModel> T.querySortedAsSingle(
    fieldName: List<String>,
    order: List<Sort>,
    query: Query<T>? = null
) = singleQuery(fieldName, order, query)

inline fun <reified T : RealmModel> querySortedAsSingle(
    fieldName: List<String>,
    order: List<Sort>,
...
internal fun <T : RealmModel> performSingleQuery(
    ...
    return Single.create<List<T>> { emitter ->
        ...
        val realm =
            RealmConfigStore.fetchConfiguration(javaClass)?.realm()
