...
fun <T : RealmModel> T.queryFirstAsync(callback: (T?) -> Unit) =
    queryFirstAsync(callback, this.javaClass)

inline fun <reified T : RealmModel> queryFirstAsync(noinline callback: (T?) -> Unit) =
    queryFirstAsync(callback, T::class.java)
...
fun <T : RealmModel> T.queryAllAsync(callback: (List<T>) -> Unit) =
    queryAllAsync(callback, this.javaClass)

inline fun <reified T : RealmModel> queryAllAsync(noinline callback: (List<T>) -> Unit) =
    queryAllAsync(callback, T::class.java)
...
fun <T : RealmModel> T.queryAsync(query: Query<T>, callback: (List<T>) -> Unit) =
    queryAsync(query, callback, this.javaClass)

inline fun <reified T : RealmModel> queryAsync(
    noinline query: Query<T>,
    noinline callback: (List<T>) -> Unit
) = queryAsync(query, callback, T::class.java)

@PublishedApi
internal fun <T : RealmModel> queryAsync(
    query: Query<T>,
