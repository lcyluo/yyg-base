...
import com.yyg.common.constant.OpenUrlScheme
...
object OpenUrlUtil {
    ...
    fun openBrowser(context: Context, url: String) {
        try {
            ...
            context.toast(R.string.common_application_not_installed)
            e.printStackTrace()
        }
    }
    ...
    fun shouldInterceptScheme(context: Context, url: String): Boolean {
        ...
        // 处理苹果商店
        if (url.contains(OpenUrlScheme.APPLE)) {
    ...
    @JvmStatic
    fun convertWatcherData(data: MutableList<String>?): MutableList<Uri>? {
        ...
        return data
            ...
            .map {
                if (it.startsWith("http")) Uri.parse(it) else UriUtils.file2Uri(File(it))
            }
            .toMutableList()
    }
